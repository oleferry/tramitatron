...
def test_ask_returns_official_excerpt_with_source(kclient):
    ...
    assert "tarjeta sanitaria" in result["answer"]
    assert result["source"]["organismo"] == "Sanidad de Prueba"
    assert result["source"]["fetched_at"] == "2026-07-15"
    assert result["source"]["url"].startswith("https://ejemplo-oficial.sacyl.es")


def test_ask_itv_finds_itv_source(kclient):
    result = _ask(kclient, "¿Qué documentos hacen falta para la ITV del vehículo?")
    assert result["found"] is True
    assert result["source"]["organismo"] == "ITV de Prueba"
...
def test_ask_prefers_procedure_sources(kclient):
    ...
    result = _ask(kclient, "cita previa internet", procedure_id="jcyl.itv.info")
    assert result["source"]["organismo"] == "ITV de Prueba"
...
def test_procedure_sources_have_strict_priority(kclient):
    ...
    result = _ask(
        ...
        procedure_id="sacyl.health.primary-care",
    )
    assert result["found"] is True
    assert result["source"]["organismo"] == "Sanidad de Prueba"
